<template>
    <PortalLayout :department="department" :available-departments="availableDepartments" :is-global-admin="isGlobalAdmin" portal-type="ministry">
        <div class="py-6 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex items-center gap-3">
                    <!-- Class Type Filter -->
                    <select v-model="filterClassType" class="text-sm border-gray-200 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 py-2.5">
                        <option value="all">Tất cả loại lớp</option>
                        <option value="sunday_school">Trường Chúa Nhật</option>
                        <option value="gospel">Lớp Giáo Lý</option>
                        <option value="bible_quiz">Trắc Nghiệm KT</option>
                    </select>
            ...
            <!-- Empty state -->
            <div v-if="filteredClasses.length === 0" class="py-20 flex flex-col items-center text-center bg-white rounded-2xl border border-gray-100 shadow-sm">
            ...
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div v-for="cls in filteredClasses" :key="cls.id" class="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col">
                    <!-- Card header -->
                    <div class="p-5 flex-1">
                        <div class="flex items-start gap-2 mb-3">
                            <div class="w-10 h-10 bg-indigo-100 text-indigo-700 rounded-xl flex items-center justify-center font-bold text-base shrink-0">
                                {{ cls.name.charAt(0) }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <h3 class="font-bold text-gray-900 leading-tight truncate">{{ cls.name }}</h3>
                    ...
                    <div class="px-5 py-3 border-t border-gray-100 flex gap-2">
                        <Link :href="route(routePrefix + '.sessions', cls.id)"
                            class="flex-1 text-center text-sm font-bold py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-500 text-white rounded-xl hover:from-indigo-700 hover:to-indigo-600 transition-all shadow-sm flex items-center justify-center gap-1.5">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select v-model="form.class_type" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        <option value="sunday_school">Trường Chúa Nhật (điểm danh + tiền dâng)</option>
                        <option value="gospel">Lớp Giáo Lý / Phúc Âm (chỉ điểm danh)</option>
            ...
            <div class="space-y-4">
                ...
                <div class="flex bg-gray-100 rounded-xl p-1">
                    <button @click="memberTab = 'search'"
                        :class="memberTab === 'search' ? 'bg-white shadow text-indigo-700 font-bold' : 'text-gray-500 hover:text-gray-700'"
                        class="flex-1 text-sm py-1.5 rounded-lg transition-all font-bold">Tìm kiếm</button>
                    <button @click="memberTab = 'bulk'"
                        :class="memberTab === 'bulk' ? 'bg-white shadow text-indigo-700 font-bold' : 'text-gray-500 hover:text-gray-700'"
                ...
                <div v-if="memberTab === 'search'">
                    ...
                    <div v-if="selectedMember" class="mt-2 flex items-center gap-2 p-2.5 bg-indigo-50 rounded-xl">
                        ...
                        <button @click="addMember" class="px-3 py-1.5 bg-indigo-600 text-white text-xs font-bold rounded-lg hover:bg-indigo-700 transition-colors">Thêm</button>
                    </div>
                </div>
                ...
                <div v-if="memberTab === 'bulk'" class="space-y-3">
                    ...
                    <button @click="bulkAddMembers" :disabled="bulkSelected.size === 0 || bulkLoading"
                        class="w-full py-3 bg-gradient-to-r from-indigo-600 to-indigo-500 text-white font-bold rounded-xl hover:from-indigo-700 hover:to-indigo-600 transition-all disabled:opacity-50 flex items-center justify-center gap-2 text-sm">
...
const filteredClasses = computed(() => {
    if (filterClassType.value === 'all') return props.classes;
    return props.classes.filter(c => c.class_type === filterClassType.value);
});
...
const classTypeLabel = (type) => {
    const labels = {
        sunday_school: 'Trường Chúa Nhật',
        gospel: 'Giáo Lý / Phúc Âm',
        bible_quiz: 'Trắc Nghiệm KT'
    };
