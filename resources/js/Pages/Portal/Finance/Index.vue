<template>
    <PortalLayout :department="department" :available-departments="availableDepartments" :is-global-admin="isGlobalAdmin" @open-switcher="isSwitchOpen = true">
        ...
        <div class="py-4 space-y-5 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <!-- Attendance & Meetings counts -->
            <div class="grid grid-cols-3 gap-3">
                <div class="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
                    <p class="text-xs font-bold text-gray-500">Số buổi nhóm</p>
                    <p class="text-2xl font-black text-gray-900">{{ summary.meeting_count }}</p>
                </div>
                <div class="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
                    <p class="text-xs font-bold text-gray-500">TB Hiện diện</p>
                    <p class="text-2xl font-black text-gray-900">{{ summary.avg_attendance }}</p>
                </div>
                <div class="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
                    <p class="text-xs font-bold text-gray-500">Biến động HD</p>
                    <div class="flex items-baseline gap-1 mt-1">
                        <p class="text-2xl font-black" :class="summary.attendance_change >= 0 ? 'text-emerald-600': 'text-rose-600'">
                            {{ summary.attendance_change >= 0 ? '+' : '' }}{{ summary.attendance_change }}%
                        </p>
                        <span class="text-xs text-gray-400">so tháng trước</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-5 py-3.5 border-b border-gray-100 flex justify-between items-center">
                    <h3 class="text-sm font-bold text-gray-900">Danh sách Buổi Nhóm</h3>
                    <span class="text-xs text-gray-500">Tháng {{ localMonth }}/{{ localYear }}</span>
                </div>
                ...
                <div class="hidden md:block overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-5 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Ngày</th>
                                <th class="px-5 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">HD Sáng</th>
                                <th class="px-5 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">HD Chiều</th>
                                <th class="px-5 py-3 text-right text-xs font-bold text-gray-500 uppercase tracking-wider">Thu (+)</th>
                                <th class="px-5 py-3 text-right text-xs font-bold text-gray-500 uppercase tracking-wider">Chi (-)</th>
                                <th class="px-5 py-3 text-right text-xs font-bold text-gray-500 uppercase tracking-wider">Tồn Tuần</th>
                                <th class="px-5 py-3 w-12"></th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-100">
                            <tr v-for="m in meetings" :key="m.id" class="hover:bg-gray-50 transition-colors group">
                                <td class="px-5 py-3.5 text-sm font-bold text-gray-900">{{ formatDate(m.meeting_date) }}</td>
                                <td class="px-5 py-3.5 text-center">
                                    <span class="inline-flex items-center gap-1 text-sm font-bold text-gray-700">
                                        <svg class="w-3.5 h-3.5 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707"/></svg>
                                        {{ m.attendance_morning }}
                                    </span>
                                </td>
                                <td class="px-5 py-3.5 text-center">
                                    <span class="inline-flex items-center gap-1 text-sm font-bold text-gray-700">
                                        ...
                                        {{ m.attendance_afternoon }}
                                    </span>
                                </td>
                                <td class="px-5 py-3.5 text-right text-sm font-bold">
                                    <span v-if="m.session_income > 0" class="text-emerald-600">+{{ formatCurrency(m.session_income) }}</span>
                                    <span v-else class="text-gray-300">—</span>
                                </td>
                                <td class="px-5 py-3.5 text-right text-sm font-bold">
                                    <span v-if="m.session_expense > 0" class="text-rose-600">-{{ formatCurrency(m.session_expense) }}</span>
                                    <span v-else class="text-gray-300">—</span>
                                </td>
                                <td class="px-5 py-3.5 text-right text-sm font-black" :class="m.session_balance >= 0 ? 'text-blue-700' : 'text-rose-700'">
                                    {{ formatCurrency(m.session_balance) }}
                                </td>
                                <td class="px-5 py-3.5 text-right">
                                    <button v-if="canManage" @click="openMeetingForm(m)" class="text-gray-300 hover:text-indigo-600 opacity-0 group-hover:opacity-100 transition-all">
                            ...
                            <!-- Totals Row -->
                            <tr v-if="meetings.length > 0" class="bg-blue-50 font-black">
                                <td class="px-5 py-3 text-sm text-blue-900">Tổng kết tháng</td>
                                <td class="px-5 py-3 text-center text-sm text-blue-900">—</td>
                                <td class="px-5 py-3 text-center text-sm text-blue-900">TB: {{ summary.avg_attendance }}</td>
                                <td class="px-5 py-3 text-right text-sm text-emerald-700">+{{ formatCurrency(summary.month_income) }}</td>
                                <td class="px-5 py-3 text-right text-sm text-rose-700">-{{ formatCurrency(summary.month_expense) }}</td>
                                <td class="px-5 py-3 text-right text-sm text-blue-900">{{ formatCurrency(summary.closing_balance) }}</td>
                                <td></td>
                            </tr>
                ...
                <div class="md:hidden divide-y divide-gray-100">
                    <div v-for="m in meetings" :key="m.id" class="p-4" @click="canManage && openMeetingForm(m)">
                        <div class="flex justify-between items-start">
                            <div>
                                <p class="text-sm font-black text-gray-900">{{ formatDate(m.meeting_date) }}</p>
                                <p class="text-xs text-gray-500 mt-0.5">☀ {{ m.attendance_morning }} · 🌙 {{ m.attendance_afternoon }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-sm font-black" :class="m.session_balance >= 0 ? 'text-blue-700' : 'text-rose-700'">{{ formatCurrency(m.session_balance) }}</p>
                                <p class="text-xs text-gray-400">Tồn tuần</p>
                            </div>
                        </div>
                        <div class="mt-2 flex gap-3 text-xs">
                            <span class="text-emerald-600 font-bold">+{{ formatCurrency(m.session_income) }}</span>
                            <span class="text-gray-300">·</span>
                            <span class="text-rose-600 font-bold">-{{ formatCurrency(m.session_expense) }}</span>
                        </div>
                    </div>
                    <div v-if="meetings.length === 0" class="p-8 text-center text-sm text-gray-500">Chưa có buổi nhóm nào.</div>
                </div>
            </div>

            <!-- Monthly Summary Card (Report) -->
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="p-5 space-y-4">
                    <table class="w-full text-sm">
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr>
                                <td class="py-2.5 text-gray-600 font-medium text-emerald-700">Tổng thu trong tháng</td>
                                <td class="py-2.5 text-right font-bold text-emerald-700">+ {{ formatCurrency(summary.month_income) }}</td>
                            </tr>
                            <tr>
                                <td class="py-2.5 text-gray-600 font-medium text-rose-700">Tổng chi trong tháng</td>
                                <td class="py-2.5 text-right font-bold text-rose-700">- {{ formatCurrency(summary.month_expense) }}</td>
                            </tr>
                            <tr class="font-black text-base">
                                <td class="py-3 text-blue-900">Tổng tồn hiện tại</td>
                                <td class="py-3 text-right text-blue-900">{{ formatCurrency(summary.closing_balance) }}</td>
                            </tr>
                        </tbody>
                    </table>
        ...
        <!-- === Meeting SlideOver === -->
        <SlideOver :show="showMeetingForm" @close="closeMeetingForm" :title="meetingFormEditing ? 'Cập nhật Buổi Nhóm' : 'Thêm Buổi Nhóm'" :wide="true">
            <form @submit.prevent="submitMeeting" class="space-y-5">
                ...
                <div class="border-t border-gray-100 pt-4 space-y-3">
                    <div class="flex justify-between items-center">
                        ...
                        <button type="button" @click="addTransaction" class="text-xs font-bold text-blue-600 hover:text-blue-800">+ Thêm dòng</button>
                    </div>
                    ...
                    <div v-for="(tx, idx) in meetingForm.transactions" :key="idx" class="bg-gray-50 rounded-xl p-3 space-y-2 border border-gray-100">
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                ...
                                <div class="flex gap-1.5">
                                    <button type="button" @click="tx.type = 'income'" class="flex-1 py-1 text-xs font-bold rounded-lg transition-colors" :class="tx.type === 'income' ? 'bg-emerald-600 text-white' : 'bg-white border border-gray-200 text-gray-600'">Thu</button>
                                    <button type="button" @click="tx.type = 'expense'" class="flex-1 py-1 text-xs font-bold rounded-lg transition-colors" :class="tx.type === 'expense' ? 'bg-rose-600 text-white' : 'bg-white border border-gray-200 text-gray-600'">Chi</button>
                                </div>
                            </div>
                            <div>
                        ...
                        <div>
                            <input type="text" v-model="tx.description" class="block w-full rounded-lg border-gray-300 text-xs focus:ring-blue-500 py-1.5" placeholder="Ghi chú...">
                        </div>
                        <button type="button" @click="removeTransaction(idx)" class="text-[10px] text-red-500 hover:text-red-700 font-bold">✕ Xóa dòng này</button>
                    </div>

                    <div v-if="meetingForm.transactions.length === 0" class="text-center py-3">
                        <p class="text-xs text-gray-400">Chưa có giao dịch nào. Bấm "+ Thêm dòng" để ghi thu/chi.</p>
                    </div>

                    <!-- Live total preview -->
                    <div v-if="meetingForm.transactions.length > 0" class="bg-blue-50 rounded-xl px-4 py-3 border border-blue-100 flex justify-between text-sm">
                        <span class="font-bold text-blue-900">Tồn buổi nhóm:</span>
...
const meetingFormEditing = ref(null); // null = add, object = edit (the meeting)
...
const liveBalance = computed(() => {
    return meetingForm.value.transactions.reduce((acc, tx) => {
