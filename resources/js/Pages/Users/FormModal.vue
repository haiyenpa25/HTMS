...
const activeSection = ref('info'); // 'info' | 'password' | 'member'
...
<template>
  <SlideOver :modelValue="show" size="lg" @update:modelValue="(val) => { if(!val) close(); }"
             :title="isEditing ? 'Cập nhật Tài khoản' : 'Tạo Tài khoản Mới'"
             :description="isEditing ? editingUser?.email : 'Điền đầy đủ thông tin để tạo mới'">
    
      <!-- ── Tab Selector (Edit only) ── -->
      <div v-if="isEditing" class="flex border-b border-gray-100 bg-gray-50/80 mx-[-1.5rem] px-6 mb-6 -mt-2">
      ...
      <div v-if="isEditing && activeSection === 'member'" class="space-y-4 pt-2">
          ...
          <div v-if="editingUser.linked_member" class="bg-white border border-emerald-200 rounded-xl p-4 flex items-center justify-between shadow-sm hover:border-emerald-300 transition-colors">
            <div>
              <p class="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-1 shadow-none">Đang liên kết với</p>
              <a :href="`/members?search=${editingUser.linked_member.member_code}`" target="_blank" class="inline-flex items-center gap-1.5 font-black text-emerald-600 hover:text-emerald-700 transition" title="Bấm để xem hồ sơ">
                {{ editingUser.linked_member.full_name }}
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
              </a>
              <p class="text-xs text-emerald-600/80 font-mono mt-0.5">{{ editingUser.linked_member.member_code }}</p>
            </div>
            <button type="button" @click="unlinkMember" class="text-red-600 hover:text-red-700 hover:bg-red-50 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors border border-transparent shadow-none">
                Gỡ liên kết
            </button>
          </div>

          <div>
            <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1.5 mt-2">
      ...
      <template #footer v-if="activeSection !== 'member'">
        <div class="flex justify-end gap-3 w-full">
            ...
            <button @click="submit" :disabled="form.processing"
              ...
              {{ isEditing ? 'Cập nhật tài khoản' : 'Tạo mới' }}
            </button>
        </div>
      </template>
  </SlideOver>
