  ...
  >
       ...
       <div v-else class="w-full p-4 sm:p-6 lg:p-8 space-y-6 max-w-5xl mx-auto">
           ...
           <div v-if="activeRole === 'secretary'" class="grid grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              ...
              <!-- Card: Điểm danh HT (Amber — Deacon accent) -->
              <Link :href="route('deacon.attendance')" class="bg-white rounded-[1.5rem] p-5 shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center group hover:shadow-md hover:border-amber-200 transition-all active:scale-95 duration-200 relative">
                  <!-- Notification Badge -->
              ...
              <!-- Card: Báo cáo (Amber — Deacon accent) -->
              <Link :href="route('deacon.report')" class="bg-white rounded-[1.5rem] p-5 shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center group hover:shadow-md hover:border-amber-200 transition-all active:scale-95 duration-200 relative">
                  <!-- Notification Badge -->
              ...
              <!-- MAC: Card Thành viên (nếu được bật cho Chấp sự) -->
              <Link v-if="deptFeatures && (deptFeatures['members'] || deptFeatures['thanh-vien'])" :href="route('ministry.members.index')" 
                  ...
                  <h3 class="font-bold text-gray-900 leading-tight">Thành viên<br><span class="text-xs font-medium" :class="(!authPermissions || !(authPermissions['members'] || authPermissions['thanh-vien'])) ? 'text-gray-400' : 'text-blue-600'">Thông diễn ban viên</span></h3>
              </Link>
           </div>
           
           <!-- Grid Cards for Treasurer -->
           <div v-if="activeRole === 'treasurer'" class="grid grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              <!-- Card: Funds (Amber — Deacon accent) -->
              <Link v-if="route().has('finance.funds.index')" :href="route('finance.funds.index')" class="bg-white rounded-[1.5rem] p-5 shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center group hover:shadow-md hover:border-amber-200 transition-all active:scale-95 duration-200 relative">
                  <div class="w-16 h-16 mb-3 rounded-2xl bg-amber-50 text-amber-500 flex items-center justify-center group-hover:bg-amber-500 group-hover:text-white transition-colors duration-300">
              ...
              <!-- Card: Finance Reports -->
              <Link v-if="route().has('finance.reports.index')" :href="route('finance.reports.index')" class="bg-white rounded-[1.5rem] p-5 shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center group hover:shadow-md hover:border-amber-200 transition-all active:scale-95 duration-200 relative">
               ...
               <div class="space-y-2">
                  ...
                  >
                     <div class="flex items-center space-x-4 shrink-0">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center font-black" :class="activeRole === dept.id ? 'bg-amber-500 text-white' : 'bg-gray-100 text-gray-600 group-hover:bg-gray-200'">
