...
<template>
  ...
  >
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 py-6 pb-20">
      ...
      <div class="flex items-start justify-between gap-4 mb-6">
        ...
        <div class="flex gap-1.5 sm:gap-2 shrink-0 no-print">
          ...
          <button @click="print" class="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-2 text-xs font-bold text-gray-600 bg-white border border-gray-200 hover:border-gray-300 rounded-xl shadow-sm">
          ...
          <button class="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-4 py-2 text-xs font-bold text-white bg-orange-500 hover:bg-orange-600 rounded-xl shadow-sm">
            <svg class="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5"/></svg>
            <span class="hidden sm:inline">Gửi thông báo</span>
          </button>
        </div>
      </div>
      ...
      <div v-if="hasMain" class="mb-10">
        <div class="flex items-center gap-3 mb-4">
          ...
          <span v-if="!canEditSectionI" class="hidden sm:flex items-center gap-1 text-[10px] font-black text-amber-600 bg-amber-50 border border-amber-200 px-2 py-1 rounded-full shrink-0">
            🔒 Chỉ xem
          </span>
        </div>
    ...
    <!-- Fixed bottom bar -->
    <div class="fixed bottom-0 inset-x-0 z-30 bg-white/95 backdrop-blur-md border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] px-3 sm:px-5 py-2.5 no-print">
      <div class="max-w-6xl mx-auto flex items-center justify-between gap-2">
        ...
        <div class="flex gap-2 shrink-0">
          ...
          <button @click="print" class="px-3 sm:px-5 py-2 bg-indigo-600 text-white font-bold text-xs rounded-xl hover:bg-indigo-700 shadow-sm">In</button>
        </div>
      </div>
    </div>
