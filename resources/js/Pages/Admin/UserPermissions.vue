    ...
    <div class="min-h-screen bg-slate-950 text-white">
      <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 border-b border-slate-700">
          <nav class="-mb-px flex space-x-6 overflow-x-auto">
            <Link :href="route('users.index')"
              class="whitespace-nowrap border-b-2 py-4 px-1 text-sm font-medium border-transparent text-slate-400 hover:text-white hover:border-slate-500 transition-colors">
              👥 Tài khoản
            </Link>
            ...
            <Link :href="route('admin.users.permissions')"
              class="whitespace-nowrap border-b-2 py-4 px-1 text-sm font-bold border-blue-500 text-blue-400">
              🔐 Cấp Quyền
            </Link>
          </nav>
        </div>
        ...
        <div class="flex gap-5 h-[calc(100vh-180px)] min-h-[600px]">
          ...
          <!-- ── LEFT: User List ── -->
          <div class="w-80 flex-shrink-0 flex flex-col bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
            <!-- Search -->
            <div class="p-4 border-b border-slate-800">
              <div class="relative">
                ...
                <input v-model="searchInput" @input="handleSearch" type="text"
                  ...
                  class="w-full pl-9 pr-3 py-2.5 text-sm bg-slate-800 border border-slate-700 text-white placeholder-slate-500 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition" />
              </div>
            </div>

            <!-- Users -->
            <div class="flex-1 overflow-y-auto p-2 space-y-1 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-700">
              <button v-for="user in users.data" :key="user.id"
                @click="selectUser(user)"
                class="w-full text-left px-4 py-3 rounded-xl transition-all duration-150 group flex items-center gap-3"
                ...
                <div class="w-9 h-9 rounded-full flex items-center justify-center font-black text-sm flex-shrink-0"
                  :class="activeUser?.id === user.id ? 'bg-blue-400 text-white' : 'bg-slate-700 text-slate-300'">
                  {{ (user.name || 'U').charAt(0).toUpperCase() }}
                </div>
                <div class="overflow-hidden">
                  <div class="font-bold text-sm truncate" :class="activeUser?.id === user.id ? 'text-white' : 'text-slate-200'">{{ user.name }}</div>
                  <div class="text-xs truncate" :class="activeUser?.id === user.id ? 'text-blue-200' : 'text-slate-500'">{{ user.email }}</div>
                </div>
              </button>
              <div v-if="!users.data.length" class="py-12 text-center text-slate-500 text-sm">
          ...
          <div class="flex-1 flex flex-col bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
            ...
            <!-- Empty state -->
            <div v-if="!activeUser" class="flex-1 flex flex-col items-center justify-center text-slate-500">
              <div class="w-16 h-16 rounded-full bg-slate-800 flex items-center justify-center text-3xl mb-4">👈</div>
              <p class="font-bold text-slate-300">Chọn một tài khoản</p>
              <p class="text-sm mt-1">để thiết lập quyền chi tiết</p>
            </div>
            ...
            <template v-else>
              ...
              <div class="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900 sticky top-0 z-10">
                ...
                <div class="flex items-center gap-4">
                  <div class="relative">
                    <div class="w-14 h-14 rounded-full bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center text-2xl font-black text-white shadow-lg shadow-blue-600/30">
                      {{ avatarLetter }}
                    </div>
                    <div class="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-green-500 border-2 border-slate-900"></div>
                  </div>
                  <div>
                    <h2 class="font-black text-white text-lg">{{ activeUser.name }}</h2>
                    <p class="text-sm text-slate-400">Phân quyền chi tiết tính năng</p>
                    <p v-if="memberInfo?.code" class="text-xs text-blue-400 font-mono mt-0.5">ID: {{ memberInfo.code }}</p>
                  </div>
                </div>
                <!-- Save Button -->
                <div class="flex items-center gap-3">
                  <span v-if="savedMsg" class="text-sm font-bold" :class="savedMsg.includes('Lỗi') ? 'text-red-400' : 'text-green-400'">
                    {{ savedMsg }}
                  </span>
                  <button @click="save" :disabled="isSaving"
                    class="px-6 py-2.5 bg-blue-500 hover:bg-blue-400 disabled:opacity-50 text-white font-bold text-sm rounded-xl shadow-md shadow-blue-600/30 transition-all flex items-center gap-2">
              ...
              <div class="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div class="bg-slate-800/60 rounded-2xl p-5 border border-slate-700/50">
                  <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <span class="w-5 h-5 rounded bg-rose-500/20 text-rose-400 flex items-center justify-center text-base">🌍</span>
                  ...
                  <div class="grid grid-cols-2 gap-3">
                    <label v-for="gr in globalRoleOptions" :key="gr.id"
                      ...
                      <div>
                        <div class="font-bold text-sm text-white">{{ gr.label }}</div>
                        <div class="text-xs text-slate-400 mt-0.5">{{ gr.desc }}</div>
                      </div>
                    </label>
                  </div>
                </div>
                ...
                <div class="bg-slate-800/60 rounded-2xl p-5 border border-slate-700/50">
                  <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <span class="w-5 h-5 rounded bg-amber-500/20 text-amber-400 flex items-center justify-center text-base">🏛️</span>
                    Ban Chấp Sự
                  </h3>
                  <label @click="toggleChurch"
                    class="flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all"
                    :class="hasChurch ? 'border-amber-500/50 bg-amber-500/10' : 'border-slate-700 hover:border-slate-600 hover:bg-slate-800'">
                    <div class="w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all"
                      :class="hasChurch ? 'bg-amber-500 border-amber-500' : 'border-slate-600'">
                      <svg v-if="hasChurch" class="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/>
                      </svg>
                    </div>
                    <span class="font-bold text-sm text-white">Có quyền Ban Chấp Sự</span>
                  </label>
                  <div v-if="hasChurch" class="mt-3 ml-8 flex items-center gap-3">
                    <span class="text-xs text-slate-400">Chức vụ:</span>
                    <select :value="churchMembership?.org_role_id"
                      @change="(e) => updateChurchRole(e.target.value)"
                      class="text-sm bg-slate-700 border border-slate-600 text-white rounded-lg px-3 py-1.5 focus:ring-2 focus:ring-amber-500 outline-none">
                ...
                <div class="bg-slate-800/60 rounded-2xl p-5 border border-slate-700/50">
                  <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <span class="w-5 h-5 rounded bg-blue-500/20 text-blue-400 flex items-center justify-center text-base">🏃</span>
                    Quyền Ban Ngành
                  </h3>

                  <!-- Active Department Chips -->
                  <div class="mb-4">
                    <p class="text-xs text-slate-500 mb-3">Quyền đã cấp:</p>
                    <div class="flex flex-wrap gap-2">
                      <button v-for="chip in activeChips" :key="chip.id"
                        ...
                        :class="activeChipId === chip.id
                          ? 'bg-blue-600 border-blue-500 text-white shadow-lg shadow-blue-600/30'
                          : 'bg-slate-700 border-slate-600 text-slate-300 hover:border-slate-500'">
                        <span class="w-4 h-4 rounded-full flex items-center justify-center text-[10px]"
                          :class="chip.block === 'activities' ? '🏃' : '🙏'">
                          {{ chip.block === 'activities' ? '🏃' : '🙏' }}
                        </span>
                        {{ chip.name }}
                        <svg @click.stop="toggleDept(chip.id)" class="w-3 h-3 ml-1 text-current opacity-60 hover:opacity-100" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                          <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
                        </svg>
                      </button>
                      <span v-if="activeChips.length === 0" class="text-xs text-slate-600 italic">Chưa có ban ngành nào</span>
                    </div>
                  </div>

                  <!-- Accordion: Active chip permissions -->
                  <div v-if="activeChipDept" class="mb-5 rounded-xl bg-slate-700/50 border border-slate-600 overflow-hidden">
                    <div class="flex items-center justify-between px-4 py-3 bg-slate-700/80">
                      <div class="flex items-center gap-2">
                        ...
                        <span class="font-black text-white text-sm">{{ activeChipDept.name }}</span>
                        <span class="text-[10px] px-2 py-0.5 rounded-full bg-blue-500/30 text-blue-300 font-bold">ACTIVE</span>
                      </div>
                      <select :value="activeChipDept.membership.org_role_id"
                        @change="(e) => updateDeptRole(activeChipDept.id, e.target.value)"
                        class="text-xs bg-slate-600 border border-slate-500 text-white rounded-lg px-2 py-1 focus:ring-2 focus:ring-blue-500 outline-none">
                    ...
                    <div class="divide-y divide-slate-700/50">
                      <label v-for="perm in featurePerms" :key="perm.key"
                        ...
                        class="flex items-center justify-between px-5 py-3.5 hover:bg-slate-700/50 cursor-pointer transition-colors">
                        <div class="flex items-center gap-3">
                          <span class="text-lg w-7">{{ perm.icon }}</span>
                          <span class="text-sm font-medium text-slate-200">{{ perm.label }}</span>
                        </div>
                        <!-- Checkbox -->
                        <div class="w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all"
                          :class="activeChipDept.membership.permissions?.[perm.key]
                  ...
                  <!-- Other chips accordion (collapsed) -->
                  <div v-for="chip in activeChips.filter(c => c.id !== activeChipId)" :key="'o' + chip.id"
                    class="mb-3 rounded-xl border border-slate-700 overflow-hidden">
                    <button @click="activeChipId = chip.id"
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <span class="font-bold text-slate-300 text-sm">{{ chip.name }}</span>
                      </div>
                  ...
                  <!-- Divider -->
                  <div class="my-4 border-t border-slate-700/50 pt-4">
                    <p class="text-xs text-slate-500 mb-3">Thêm ban ngành:</p>
                    <!-- Add from Activities -->
                    <div class="mb-3">
                      <p class="text-[10px] font-bold text-emerald-500 uppercase tracking-wider mb-2">🏃 Ban Sinh Hoạt</p>
                      <div class="flex flex-wrap gap-2">
                    ...
                    <!-- Add from Ministry -->
                    <div>
                      <p class="text-[10px] font-bold text-purple-400 uppercase tracking-wider mb-2">🙏 Ban Mục Vụ</p>
                      <div class="flex flex-wrap gap-2">
