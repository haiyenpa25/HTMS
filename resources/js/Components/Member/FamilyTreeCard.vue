<template>
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-blue-100 shadow-sm overflow-hidden">
      <div class="px-5 py-4 border-b border-blue-100 bg-blue-50/50 flex items-center justify-between">
        ...
        <span v-if="member.household" class="text-xs font-bold text-blue-700 bg-blue-100 px-2 py-0.5 rounded-md">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div v-for="m in member.household.members" :key="'hh-'+m.id" 
            ...
            <button v-if="isPastor && m.id !== member.household.head_member_id" 
              @click="setHead(m.id)"
              class="px-2.5 py-1.5 text-xs font-bold text-amber-700 bg-white border border-amber-200 hover:bg-amber-100 rounded-lg transition-colors whitespace-nowrap shadow-sm">
              Chọn Chủ hộ
            </button>
          </div>
        </div>
      </div>
      <div v-else class="p-8 text-center text-sm text-gray-400 italic">
        Thành viên này chưa được xếp vào Hộ Gia Đình nào.
      </div>
    </div>
